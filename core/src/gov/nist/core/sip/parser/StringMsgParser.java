...
import gov.nist.core.sip.header.NameMap;
 ...
 * when you have an entire SIP message or Header or SIP URL in memory and you
 ...
 * parseMessage(byte[]) else use parseSIPMessage(String) The payload is
 * accessible from the parsed message using the getContent and getContentBytes
 * methods provided by the Message class. Currently only eager parsing of the
 * message is supported (i.e. the entire message is parsed in one feld swoop).
 * 
 * 
...
public class StringMsgParser {
	...
	protected byte[] getBodyAsBytes() {
		if (this.contentLength == 0) {
			...
			System.arraycopy(currentMessageBytes, bufferPointer, body, 0,
					body.length);
			bufferPointer = endIndex;
			this.contentLength = 0;
			return body;
	...
	/**
	 * Return the contents till the end of the buffer (this is useful when you
	 * encounter an error.
	 */
	...
	/**
	 * Return tbe bytes to the end of the message. This is invoked when the
	 * parser is invoked with an array of bytes rather than with a string.
	 */
	...
	/**
	 * Return true if the body is encoded as a string. If the
	 * parseMessage(String) method is invoked then the body is assumed to be a
	 * string.
	 */
	 ...
	 * @return a Message[] structure (request or response) containing the parsed
	 *         SIP message.
	 * @exception SIPIllegalMessageException
	...
	public Message parseSIPMessage(byte[] msgBuffer) throws ParseException {
		...
		if (readBody && sipmsg.getContentLengthHeader() != null
			...
			this.contentLength = sipmsg.getContentLengthHeader()
					.getContentLength();
	 ...
	 * Message parsed structures. Note that the current limitation is that this
	 * does not handle content encoding properly. The message content is just
	 * assumed to be encoded using the same encoding as the sip message itself
	 * (i.e. binary encodings such as gzip are not supported).
	 * 
	 * @param sipMessages
	...
	/**
	 * This is called repeatedly by parseMessage to parse the contents of a
	 * message buffer. This assumes the message already has continuations etc.
	 * taken care of. prior to its being called.
	 */
	private Message parseMessage(String currentMessage) throws ParseException {
		// position line counter at the end of the sip messages.
		...
		if (!firstLine.startsWith(SIPConstants.SIP_VERSION_STRING)) {
			...
			try {
				...
				if (this.parseExceptionListener != null)
					this.parseExceptionListener.handleException(ex, sipmsg,
							new RequestLine().getClass(), firstLine,
							currentMessage);
				else
					throw ex;
			...
			try {
				...
				if (this.parseExceptionListener != null) {
					this.parseExceptionListener.handleException(ex, sipmsg,
							new StatusLine().getClass(), firstLine,
							currentMessage);
				} else
					throw ex;
		...
		for (int i = 1; i < messageHeaders.size(); i++) {
			...
			try {
				...
				this.parseExceptionListener.handleException(ex, sipmsg, null,
			...
			try {
				...
				if (this.parseExceptionListener != null) {
					...
					this.parseExceptionListener.handleException(ex, sipmsg,
							hdrClass, hdrstring, currentMessage);
