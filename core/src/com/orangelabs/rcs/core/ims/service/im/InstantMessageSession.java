...
public abstract class InstantMessageSession extends ImsServiceSession implements MsrpEventListener {
	...
	public InstantMessageSessionListener getListener() {
		return (InstantMessageSessionListener)super.getListener();
	}
	...
	public void msrpDataReceived(byte[] data, String mimeType) {
    	...
    	try {
		    // Notify listener
	    	// TODO : how to get remote contact in case of 1-n session -> use CPIM object
    		...
    		if (MimeManager.isTextType(mimeType)){
			    ...
			    // Notify listener
			    getListener().handleReceiveMessage(message);			    
    		} else
	    	if (MimeManager.isPictureType(mimeType)){
		    	...
		    	// Notify listener
	    		getListener().handleReceivePicture(getRemoteContact(), date, content);	    		
	    	} else
	    	if (MimeManager.isVideoType(mimeType)){
		    	...
		    	// Notify listener
	    		getListener().handleReceiveVideo(getRemoteContact(), date, content);
	    	} else {
	    		// Create file content
		    	...
		    	// Notify listener
	    		getListener().handleReceiveFile(getRemoteContact(), date, content);
	    	}
    	} catch(Exception e) {
	   		if (logger.isActivated()) {
	...
	public void msrpTransferAborted() {
    	...
    	// Notify listener
    	getListener().handleImError(new InstantMessageError(InstantMessageError.MSG_TRANSFER_FAILED));
	}	
	...
	public void msrpTransferError(String error) {
    	...
    	// Notify listener
    	getListener().handleImError(new InstantMessageError(InstantMessageError.MSG_TRANSFER_FAILED, error));
    }
	...
	public void sendMessage(String msg) {
		try {
	    	...
	    	// Notify listener
	   		getListener().handleImError(new InstantMessageError(InstantMessageError.MSG_TRANSFER_FAILED, e.getMessage()));
		}
	}
