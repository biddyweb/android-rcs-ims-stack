	...
	public void run() {
		try {
			...
			while(!terminated) {
				...
				if (logger.isActivated()) {
					logger.debug("Parse transaction ID: " + txId);
				}
				...
				stream.read();
				if (logger.isActivated()) {
					logger.debug("Parse method: " + method);
				}
				...
				for (i = stream.read(); (i != MsrpConstants.CHAR_LF) && (i != -1);) {
					...
					if (logger.isActivated()) {
						logger.debug("Parse header: " + headerName + " " + headerValue);
					}
				...
				stream.read();
				if (logger.isActivated()) {
					logger.debug("Parse header terminated");
				}
				...
				if (responseCode != -1) {
					...
					if (logger.isActivated()) {
						logger.debug("MSRP response received:\n" + chunk.toString());
					}
					...
					if (method.toString().equals(MsrpConstants.METHOD_SEND)) {
						...
						byte data[] = null;
						long totalSize = 0;
						// The byte range header may not be present (empty packet for example)
						...
						if (data!=null){
							...
							if (logger.isActivated()) {
								logger.debug("EMPTY SEND request received");
							}
						}
						if (logger.isActivated()) {
					...
					if (method.toString().equals(MsrpConstants.METHOD_REPORT)) {
						...
						if (logger.isActivated()) {
							logger.debug("REPORT request received:\n" + chunk.toString());
						}
