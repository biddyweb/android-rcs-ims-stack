...
public class ImsConnectionManager implements Runnable {
        ...
        public void onReceive(Context context, Intent intent) {
            if (intent.getAction().equals(ConnectivityManager.CONNECTIVITY_ACTION)) {
				...
				if ((netInfo != null) && (netInfo.isConnected())) {
    				...
    				String defAccess = imsModule.getCore().getConfig().getString("NetworkAccess", "");
    				if ((defAccess.length() > 0) && (!defAccess.equalsIgnoreCase(netInfo.getTypeName()))) {
    					if (logger.isActivated()) {
    				...
    				String ope = tm.getSimOperatorName();
    				String defOpe = imsModule.getCore().getConfig().getString("Operator", "");
    				if ((defOpe.length() > 0) && !ope.equalsIgnoreCase(defOpe)) {
    					if (logger.isActivated()) {
    				...
    				String defApn = imsModule.getCore().getConfig().getString("APN", "");
    				if ((defApn.length() > 0) && !defApn.equalsIgnoreCase(apn)) {
    					if (logger.isActivated()) {
	...
	public void run() {
		...
		long registerPollingPeriod = imsModule.getCore().getConfig().getInteger("ImsConnectionPollingPeriod", 30);
		long servicePollingPeriod = imsModule.getCore().getConfig().getInteger("ImsServicePollingPeriod", 300);
