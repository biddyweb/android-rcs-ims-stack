...
public class OriginatingLargeInstantMessageSession extends ImsServiceSession implements MsrpEventListener {
	...
	public void handle200OK(SipResponse resp) {
		try {
	        ...
	        msrpMgr.sendChunks(stream, "text/plain", txt.length());
		} catch(Exception e) {
        	if (logger.isActivated()) {
        		logger.error("Session initiation has failed", e);
        	}
	...
	public void handleError(InstantMessageError error) {
        ...
        // Notify listener
    	getImsService().getImsModule().getCore().getListener().handleSendInstantMessageFailed(error);
	}
	...
	public void receiveBye(SipRequest bye) {
        ...
        // Notify listener
    	getImsService().getImsModule().getCore().getListener().handleSendInstantMessageFailed(
    			new InstantMessageError(InstantMessageError.SESSION_TERMINATED_BY_REMOTE));
	}
	...
	public void msrpDataTransfered() {
    	...
    	// Notify listener
    	getImsService().getImsModule().getCore().getListener().handleSendInstantMessageSuccessful();
	}
	...
	public void msrpTransferError(String error) {
        ...
        // Notify listener
    	getImsService().getImsModule().getCore().getListener().handleSendInstantMessageFailed(
    			new InstantMessageError(InstantMessageError.MSG_TRANSFER_FAILED, error));
	}
