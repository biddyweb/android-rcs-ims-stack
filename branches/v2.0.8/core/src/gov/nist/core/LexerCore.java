...
public abstract class LexerCore extends StringTokenizer {
	...
	/**
	 * Match the given token or throw an exception if no such token can be
	 * matched.
	 */
		...
		if (tok > START && tok < END) {
			...
			if (tok == DIGIT) {
				if (!isDigit(next))
					throw new ParseException(buffer + "\nExpecting DIGIT", ptr);
				this.currentMatch = new Token();
				this.currentMatch.tokenValue = new StringBuffer().append(next)
						.toString();
				this.currentMatch.tokenType = tok;
				consume(1);
			...
			if (next == ch.charValue()) {
				this.currentMatch = new Token();
				this.currentMatch.tokenValue = new StringBuffer().append(
						ch.charValue()).toString();
				this.currentMatch.tokenType = tok;
				consume(1);
			} else
				throw new ParseException(buffer + "\nExpecting  "
	...
	public boolean startsId() {
		try {
			...
			return (isAlpha(nextChar) || isDigit(nextChar) || nextChar == '_'
					|| nextChar == '+' || nextChar == '-' || nextChar == '!'
					|| nextChar == '`' || nextChar == '\'' || nextChar == '%' || // added
																					// by
																					// Bruce
																					// Konik
					nextChar == '~' || nextChar == '.' || nextChar == '*');
	...
	/**
	 * Lookahead in the inputBuffer for n chars and return as a string. Do not
	 * consume the input.
	 */
	...
	public String number() throws ParseException {
		...
		try {
			if (!isDigit(lookAhead(0))) {
				throw new ParseException(buffer + ": Unexpected token at "
						+ lookAhead(0), ptr);
			}
	...
	public String getString(char c) throws ParseException {
		...
		while (true) {
			...
			if (next == '\0') {
				ParseException exception = new ParseException(this.buffer
