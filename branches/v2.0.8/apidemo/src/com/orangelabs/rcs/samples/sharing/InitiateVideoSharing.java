...
public class InitiateVideoSharing extends Activity implements SurfaceHolder.Callback {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.richcall_initiate_video_sharing);
            ...
            Thread thread = new Thread() {
            	public void run() {
                	try {
                		...
                		e.printStackTrace();
	    		    	Utils.showError(InitiateVideoSharing.this, getString(R.string.label_invitation_failed));
	            	}
            	}
            };
            thread.start();
    ...
    public void surfaceChanged(SurfaceHolder holder, int format, int w, int h) {
        ...
        p.setPictureFormat(PixelFormat.YCbCr_420_SP);
        p.setPreviewFormat(PixelFormat.YCbCr_420_SP);
        p.setPreviewFrameRate(8);
    ...
    private IVideoSharingEventListener cshSessionListener = new IVideoSharingEventListener.Stub() {
		...
		public void handleSessionAborted() {
			handler.post(new Runnable() { 
				public void run() {
					Utils.showInfo(InitiateVideoSharing.this, getString(R.string.label_sharing_aborted));
				}
			});
		}
		...
		public void handleSessionTerminated() {
			handler.post(new Runnable() { 
				public void run() {
					Utils.showInfo(InitiateVideoSharing.this, getString(R.string.label_sharing_terminated));
				}
			});
		}
		...
		public void handleSessionTerminatedByRemote() {
			handler.post(new Runnable() { 
				public void run() {
					Utils.showInfo(InitiateVideoSharing.this, getString(R.string.label_sharing_terminated_by_remote));
				}
			});
		}
			...
			handler.post(new Runnable() { 
				public void run() {
					if (error == ContentSharingError.SESSION_INITIATION_DECLINED) {
    					Utils.showInfo(InitiateVideoSharing.this, getString(R.string.label_invitation_declined));
					} else {
    					Utils.showError(InitiateVideoSharing.this, getString(R.string.label_invitation_failed));
					}
				}
