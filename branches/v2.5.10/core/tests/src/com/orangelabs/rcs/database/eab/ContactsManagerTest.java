package com.orangelabs.rcs.database.eab;
...
public class ContactsManagerTest extends AndroidTestCase {
	...
	private void contactCreate(String numb) {
        ContactInfo info = new ContactInfo();
        info.setContact(numb);
	        info.setRcsStatus(ContactInfo.NOT_RCS);
	         info.setRcsStatusTimestamp(1354874240);
	        info.setRegistrationState(ContactInfo.REGISTRATION_STATUS_UNKNOWN);
	        try {
	            cm.setContactInfo(info, null);
	        } catch (ContactsManagerException e) {
	            if (logger.isActivated()){
    ...
    public void testGetRcsContactInfo() {
        // Get contact info
    	contactoCreate();
       ContactInfo getInfo = cm.getContactInfo(contacto);
        if (logger.isActivated()) {
            logger.debug("Get RCS contact info " + contacto);
        }
...
// create then change a RCSContact into a basic Contact then return it to be a RCSContact
    	List<String> rcscontacts = cm.getRcsContacts();
        if (logger.isActivated()){
        	for(String rcs : rcscontacts) {
            logger.debug("RCS contact : " + rcs);
        	}
        }
       	if(rcscontacts.isEmpty()) contactoCreate();
           	
    	ContactInfo oldinfo = cm.getContactInfo(contacto);
 ...
 // if (new)PresenceInfo is null, error on ContactManager line 504 so  
	 ...
	 List<String> contacts = cm.getAllContacts();
	 List<String> avails = cm.getAvailableContacts();
	 boolean contactChange = (contacts.contains(contacto) && (!(rcscontacts.contains(contacto))));
    if (logger.isActivated()){
    	 if(rcscontacts.isEmpty()) {
 	    	      logger.debug("no RCS contact ");
    	 } else {
        	for(String rcs : rcscontacts) {
        		logger.debug("RCS contact : " + rcs);
        	}
        	for(String av : avails) {
        		logger.debug("available contact : " + av);
        	}
         }
     }
     assertEquals(contactChange, true); 
     
     oldinfo = cm.getContactInfo(contacto);
    ...
    capa.setImSessionSupport(true);
	 ...
	 rcscontacts = cm.getRcsContacts();
	 avails = cm.getAvailableContacts();
	 contacts = cm.getAllContacts();
	boolean contactToRCS = (rcscontacts.contains(contacto) && contacts.contains(contacto));
	if (logger.isActivated()){
		if(rcscontacts.isEmpty()) {
	    	      logger.debug("no RCS contact ");
		} else {
...
/*    
    ...
    public long TestGetAssociatedRcsRawContact(final long rawContactId, final String rcsNumber) {
    }
    public boolean TestIsOnlySimAssociated(final String phoneNumber) {}
    public boolean TestIsSimAssociated(final long rawContactId){}
    public boolean TestIsSimAccount(final long rawContactId){}
    private List<Long> TestGetRcsRawContactIdsFromContact(final String contact){}
    public long TestCreateRcsContact(final ContactInfo info, final long rawContactId) {}
    public void testSetContactPhotoIcon(String contact, PhotoIcon photoIcon)  {
    }
    public long TestCreateMyContact() {}
    public void TestRemoveContactPhotoIcon(String contact) {}
    public ContactInfo TestGetContactInfo(String contact){}
    public void TestSetContactSharingStatus(String contact, String status, String reason){}
	public int TestGetContactSharingStatus(String contact){}
	public void TestRevokeContact(String contact){}
    public void TestUnrevokeContact(String contact) {}
    public void TestBlockContact(String contact) {}
    public void TestUnblockContact(String contact) {}
    public void TestFlushContactProvider(){}
	public void TestModifyRcsContactInProvider(String contact, int rcsStatus){}
	public List<String> TestGetRcsContactsWithSocialPresence(){
    }
    public List<String> TestGetAllContacts(){}
    public List<String> TestGetRcsBlockedContacts(){}
    public List<String> TestGetRcsInvitedContacts(){}
    public List<String> TestGetRcsWillingContacts(){}
    public List<String> TestGetRcsCancelledContacts(){}
    public void TestRemoveCancelledPresenceInvitation(String contact){}
	public boolean TestIsNumberBlocked(String number){}
    public boolean TestIsNumberShared(String number){}
    public boolean TestIsNumberInvited(String number) {}
    public boolean TestIsNumberWilling(String number){}
	public boolean TestIsNumberCancelled(String number){}
    public boolean TestIsRcsValidNumber(String phoneNumber){}
    private ArrayList<ContentProviderOperation> TestModifyContactTypeForContact(long rawContactId,
    		String rcsNumber, int newContactType, int oldContactType){}
    private ContentProviderOperation TestModifyMimeTypeForContact(long rawContactId, String rcsNumber,
    		String mimeType, boolean newState, boolean oldState){}
    private ContentProviderOperation TestInsertMimeTypeForContact(long rawContactId, String rcsNumber,
    		String mimeType){}
    private ContentProviderOperation TestDeleteMimeTypeForContact(long rawContactId, String rcsNumber, String mimeType){}
    private ArrayList<ContentProviderOperation> TestModifyContactRegistrationState(long rawContactId,
    				String rcsNumber, int newRegistrationState, int oldRegistrationState, String newFreeText, String oldFreeText){}
    				
    private List<ContentProviderOperation> TestModifyExtensionsCapabilityForContact(long rawContactId,
    				String rcsNumber, ArrayList<String> newExtensions, ArrayList<String> oldExtensions){}
    
    private ArrayList<ContentProviderOperation> TestModifyPresenceForContact(long rawContactId, String rcsNumber,
    		PresenceInfo newPresenceInfo, PresenceInfo oldPresenceInfo){}
    							  
   private String TestGetMimeTypeDescription(String mimeType){}
    public List<String> TestGetImSessionCapableContacts() {}
    public List<String> TestGgetRichcallCapableContacts(){}
 	public List<String> TestGetAvailableContacts() {}  	
	public boolean TestIsContactRcsActive(String contact){}
    public void TestSetContactCapabilities(String contact, Capabilities capabilities, int contactType, int registrationState) {}
    public void TestSetContactCapabilities(String contact, Capabilities capabilities) {}
    public Capabilities TestGetContactCapabilities(String contact){}
    public void TestSetContactCapabilitiesTimestamp(String contact, long timestamp){}
 */ 	
