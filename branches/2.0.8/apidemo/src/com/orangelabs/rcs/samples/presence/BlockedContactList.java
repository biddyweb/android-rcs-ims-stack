 ...
 * @author jexa7410
 */
public class BlockedContactList extends ListActivity implements ClientApiListener {
	/**
	 * UI handler
	 */
		...
		Thread t = new Thread() {
        	public void run() {
				try {
        			...
        			final List<String> list = presenceApi.getBlockedContacts();
        			
        			// Display result
        			handler.post(new Runnable() { 
            			public void run() {
                			if (list.size() == 0) {
                				Utils.showInfo(BlockedContactList.this, getString(R.string.label_list_empty));
                				return;
                			}
                			setListAdapter(new ArrayAdapter<String>(BlockedContactList.this,
            		...
            		handler.post(new Runnable() { 
            			public void run() {
            				Utils.showError(BlockedContactList.this, getString(R.string.label_read_list_ko));
            			}
                	});
        		}
	    	}
