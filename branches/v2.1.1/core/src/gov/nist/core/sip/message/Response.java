...
public final class Response extends Message {
	 ...
	 * of this call (for example, a database is being consulted). This response,
	 * like all other provisional responses, stops retransmissions of an INVITE
	 * by a UAC. The 100 (Trying) response is different from other provisional
	 ...
	 * reason phrase MAY give further details about the status of the call, for
	 * example, "5 calls queued; expected waiting time is 15 minutes". The
	 ...
	 * re-sending a status code from an asynchronous operation such as this. The
	 * 202 response is intentionally non-committal. Its purpose is to allow a
	 ...
	 * characteristics and location(s) from which the user or UA can choose the
	 ...
	 * automatic redirection or MAY ask the user to confirm a choice. However,
	 ...
	 * several different locations and the server cannot or prefers not to proxy
	 * the request.
	 */
	 ...
	 * and registrars, while 407 (Proxy Authentication Required) is used by
	 * proxy servers.
	 */
	 ...
	 * request. If the condition is temporary, the server SHOULD include a
	 ...
	 * method. The server MUST return a list of acceptable formats using the
	 * Accept, Accept-Encoding, or Accept-Language header field, depending on
	 * the specific problem with the content.
	 */
	 ...
	 * the unsupported extensions in an Unsupported header field in the
	 * response.
	 */
	...
	/**
	 * This status indicates that the UAS received a request that does not match
	 ...
	 * alternatives can infringe on privacy of the user or the organization. It
	 * MUST be possible to configure a server to respond with status 404 (Not
	 * Found) or to suppress the listing of possible choices for ambiguous
	 ...
	 * such as through a voice mail service. Status 600 (Busy Everywhere) SHOULD
	 ...
	 * request may succeed elsewhere. A message body containing a description of
	 * media capabilities MAY be present in the response, which is formatted
	 * according to the Accept header field in the INVITE (or application/sdp if
	 * not present), the same as a message body in a 200 (OK) response to an
	 * OPTIONS request.
	 * 
	 * @since v1.1
	 */
	...
	/**
	 * The Bad Event extension response code is used to indicate that the server
	 * did not understand the event package specified in a "Event" header field.
	 * This response code is specific to the event notification framework.
	 * 
	 * @since v1.1
	 */
	 ...
	 * condition is temporary, the server MAY indicate when the client may retry
	 * the request using the Retry-After header field.
	 */
	 ...
	 * field. If no Retry-After is given, the client MUST act as if it had
	 ...
	 * and does not wish to take the call at this time. The response MAY
	 * indicate a better time to call in the Retry-After header field. If the
	 * callee does not wish to reveal the reason for declining the call, the
	 * callee uses status code 603 (Decline) instead. This status response is
	 * returned only if the client knows that no other end point (such as a
	 ...
	 * better time to call in the Retry-After header field. This status response
	 * is returned only if the client knows that no other end point will answer
	 * the request.
	 */
	 ...
	 * style were not acceptable. A 606 (Not Acceptable) response means that the
	 * user wishes to communicate, but cannot adequately support the session
	 * described. The 606 (Not Acceptable) response MAY contain a list of
	 * reasons in a Warning header field describing why the session described
	 * cannot be supported.
	 * <p>
	 ...
	 * end point will answer the request. This specification renames this status
	 * code from NOT_ACCEPTABLE as in RFC3261 to SESSION_NOT_ACCEPTABLE due to
	 * it conflict with 406 (Not Acceptable) defined in this interface.
	 */
	...
	protected void checkHeaders() throws ParseException {
		if (getCSeqHeader() == null) {
			throw new ParseException(Header.CSEQ, 0);
		}
		if (getToHeader() == null) {
			throw new ParseException(Header.TO, 0);
		}
		if (getFromHeader() == null) {
			throw new ParseException(Header.FROM, 0);
		}
		if (getViaHeaders() == null) {
			throw new ParseException(Header.VIA, 0);
		}
	}
	 ...
	 * transmission.
	 * 
	 * @return a byte array containing the Request encoded as a byte array.
	 */
	...
	public String getDialogId(boolean isServer) {
		...
		ToHeader to = (ToHeader) this.getToHeader();
	...
	/**
	 * Create a new Request from the given response. Note that the RecordRoute
	 * Via and CSeqHeader headers are not copied from the response. These have
	 * to be added by the caller. This method is useful for generating ACK
	 * messages from final responses.
	 * 
	 * @param requestURI
	 *            is the request URI to use.
	 * @param via
	 *            is the via header to use.
	 * @param cseq
	 *            is the cseq header to use in the generated request.
	 */
	...
	public Request createRequest(URI requestURI, ViaHeader via, CSeqHeader cseq) {
		...
		if ((equalsIgnoreCase(method, Request.ACK) || equalsIgnoreCase(method,
				Request.CANCEL))
