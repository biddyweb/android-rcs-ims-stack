...
- Capability discovery optimization (thread pool).
- SIPS (SIP over TLS).
- NAT traversal support for SIP (RFC6135, draft-ietf-simple-msrp-sessmatch-11).
- Refactoring of the contacts database provider (use of batch procedures).
- Android makefile and RCS API jar file.
...
- Mantis #0012604	Core: RCS icon not grayed in air plane mode
- Mantis #0012600	RI: chat view exception
- Mantis #0012599	Core: Bad boudary tag in multipart content of the chat INVITE after a 407 response
- Mantis #0012180	Chat app: Exception if capture photo is used to initiate a file transfer
- Mantis #0012559	Core: Exception when changing a contact's telephone number
- Mantis #0011254	RI: No text entry to edit the first message before to initiate a chat session
- Mantis #0010429	Core: When a number is present more than once, only the first entry encountered is enriched with a rcs raw contact
- Mantis #0011248	Core: Request capabilities on a lot of contacts is too long
- Mantis #0012494	Core: User-agent or Server header shall start with "IM-client" text
- Mantis #0011921	Core: Notification not removed when 3G lost during a rich call
